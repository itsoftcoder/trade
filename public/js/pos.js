$(document).ready(function() {
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function() {
        ...
        var p_price = __read_number(tr.find('input.purchase_unit_price'));
        var line_total = entered_qty * unit_price_inc_tax;
        var p_total = entered_qty * p_price;

        __write_number(tr.find('input.pos_line_total'), line_total, false, 2);
        tr.find('span.pos_line_total_text').text(__currency_trans_from_en(line_total, true));
        __write_number(tr.find('input.purchase_sub_total'), p_total, false, 2);
        tr.find('span.purchase_price_total').text(__currency_trans_from_en(p_total, true));
    ...
    $('button.pos-express-finalize').click(function() {
        ...
        if (pay_method == 'card') {
            ...
            pos_form_obj.submit();
            $('.purchase_price_total').text('');
            $('.dummyAmount').val('');
            $('.dummyChangeReturn').text('');
            $('#dummyChangeReturn').val('');
        }
    });
...
function pos_total_row() {
    ...
    $('table#pos_table tbody tr').each(function() {
        ...
        purchase_total = purchase_total + __read_number($(this).find('input.purchase_sub_total'));
    });
    ...
    $('span.purchase_price_total').html(__currency_trans_from_en(purchase_total, false));
...
function calculate_billing_details(price_total) {
    ...
    $('.dummy_total_payable').val(total_payable_rounded);
    $('.dummyAmount').val(total_payable_rounded);
...
$(document).on('keyup', '.dummyAmount',function () {
    let getDummyTotal = __read_number($('#final_total_input'));
    let getDummyAmount = __read_number($('.dummyAmount'));
    let remain = getDummyAmount - getDummyTotal;
    $('.dummyChangeReturn').text(remain);
    $('#dummyChangeReturn').val(remain);
