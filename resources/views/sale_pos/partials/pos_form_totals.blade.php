    ...
    <div class="col-md-12">
        <table class="table table-condensed">
            <tr>
                ...
                <td>
                    <b>Purchase total</b> &nbsp;
                    <span class="purchase_price_total">0</span>
                </td>
            </tr>
            <tr>
                <td>
                    <b>
                        ...
                        @if($is_discount_enabled)
                            (-):
                            @if($edit_discount)
                                <i class="fas fa-edit cursor-pointer" id="pos-edit-discount" title="@lang('sale.edit_discount')" aria-hidden="true" data-toggle="modal"
                                   data-target="#posEditDiscountModal"></i>
                            @endif

                            <span id="total_discount">0</span>
                        @endif
                        <input type="hidden" name="discount_type" id="discount_type" value="@if(empty($edit)){{'percentage'}}@else{{$transaction->discount_type}}@endif"
                ...
                <td class="@if($pos_settings['disable_order_tax'] != 0) hide @endif">
					<span>
						...
						<i class="fas fa-edit cursor-pointer" title="@lang('sale.edit_order_tax')" aria-hidden="true" data-toggle="modal" data-target="#posEditOrderTaxModal"
                           id="pos-edit-tax"></i>
						<span id="order_tax">
							@if(empty($edit))
                                0
                ...
                <td>
					<span>
						...
						<input type="hidden" name="shipping_charges" id="shipping_charges"
                               value="@if(empty($edit)){{@num_format(0.00)}} @else{{@num_format($transaction->shipping_charges)}} @endif" data-default="0.00">
					</span>
                </td>
            ...
            <tr>
                <td colspan="5" style="background: deepskyblue;color: white; text-align: center">
                    <strong style="font-size: 30px;text-transform: uppercase;color: black">@lang('sale.total_payable'):</strong> <input type="hidden" name="final_total"
                                                                                                                                        id="final_total_input" value=0>
                    <span id="total_payable" class="text-white lead text-bold" style="font-size: 30px;text-transform: uppercase">0</span>
                </td>
            </tr>
        </table>
    </div>
